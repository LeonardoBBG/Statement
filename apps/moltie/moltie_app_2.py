...
DEFAULT_OUT_DIR = DEFAULT_BATCH_ROOT
DEFAULT_GROUPED_JOBS_PATH = REPO_ROOT / "output" / "grouped_jobs_df_all_modes.parquet"
DEFAULT_CSV_GROUPED_JOBS_PATH = CSV_WORKFLOW_DIR / "grouped_jobs_df_all_modes.parquet"
DEFAULT_MANUAL_GROUPED_JOBS_PATH = MANUAL_WORKFLOW_DIR / "grouped_jobs_df_all_modes.parquet"
...
def _available_csv_modes() -> list[str]:
    modes = []
    for mode in ["offensive", "defensive"]:
        required = [
            DEFAULT_MATCHES_INDEX_BY_MODE[mode],
            DEFAULT_WS_ENHANCED_BY_MODE[mode],
            DEFAULT_Y_BY_MODE[mode],
        ]
...
def _candidate_manual_source_grouped_paths(row_obj: dict, y_manual_path: Path) -> list[Path]:
    candidates = []

    explicit = row_obj.get("source_grouped_jobs_path")
    if explicit:
        candidates.append(Path(str(explicit)).expanduser())

    manual_dir = y_manual_path.expanduser().resolve().parent
    candidates.extend(
        [
            manual_dir / "grouped_jobs_df.parquet",
            manual_dir / "grouped_jobs_df_all_modes.parquet",
            DEFAULT_GROUPED_JOBS_PATH,
        ]
    )
...
def _candidate_manual_ws_enhanced_paths(row_obj: dict, y_manual_path: Path, mode: str) -> list[Path]:
    candidates = []

    explicit = row_obj.get("source_ws_enhanced_path")
    if explicit:
        candidates.append(Path(str(explicit)).expanduser())
    ...
    candidates.extend(
        [
            manual_dir / f"Leonardo_WS_enhanced_{mode}.csv",
            manual_dir / "Leonardo_WS_enhanced.csv",
            DEFAULT_WS_ENHANCED_BY_MODE[mode],
        ]
    )
...
def _select_manual_ws_enhanced_path(row_obj: dict, y_manual_path: Path, mode: str) -> Path:
    checked = []
    for path in _candidate_manual_ws_enhanced_paths(row_obj, y_manual_path, mode):
        checked.append(str(path))
        if path.exists():
            return path
    raise FileNotFoundError(
        "No WS enhanced source found for manual needle bridging. Checked: "
        + ", ".join(checked)
    )


def _source_row_numbers(source_rows: list[str]) -> list[int]:
    out = []
...
def _build_manual_bridge_run_plan(
    *,
    y_manual_path: Path,
    ...
    for mode in selected_modes:
        ...
        if not matches_index_csv.exists():
            raise FileNotFoundError(f"Missing file: {matches_index_csv}")

        pseudo_rows = []
        pseudo_y_rows = {}
        row_maps = {}
        ws_cache: dict[str, pd.DataFrame] = {}

        for i, (manual_y_row_id, row_obj) in enumerate(_iter_manual_y_records(y_manual_path), start=1):
            payload = row_obj.get("y") if isinstance(row_obj, dict) and isinstance(row_obj.get("y"), dict) else row_obj
            payload = payload if isinstance(payload, dict) else {}
            source_rows = [str(x).strip() for x in as_list(row_obj.get("source_inferred_rows")) if str(x).strip()]
            row_num_values = _source_row_numbers(source_rows)
            if not row_num_values:
                continue

            ws_path = _select_manual_ws_enhanced_path(row_obj, y_manual_path, mode)
            ws_cache_key = str(ws_path)
            ws_df = ws_cache.get(ws_cache_key)
            if ws_df is None:
                ws_df = pd.read_csv(ws_path)
                ws_cache[ws_cache_key] = ws_df
            if "X1" not in ws_df.columns:
                raise KeyError(f"Manual WS enhanced source missing X1 column: {ws_path}")

            ws_df["_row_num"] = pd.to_numeric(ws_df["X1"], errors="coerce")
            source_ws = ws_df[ws_df["_row_num"].isin(row_num_values)].copy()
            if source_ws.empty:
                raise ValueError(
                    f"Manual source_inferred_rows did not match any WS rows in {ws_path} for mode={mode}: {source_rows}"
                )

            shared_has_cols = [c for c in source_ws.columns if c.startswith("has__")]
            if not shared_has_cols:
                raise ValueError(f"No has__ needle columns found in WS enhanced source: {ws_path}")

            pseudo_row = {"X1": i}
            for col in shared_has_cols:
                vals = pd.to_numeric(source_ws[col], errors="coerce").fillna(0)
                pseudo_row[col] = int((vals > 0).any())
            if "has__any_needle" not in pseudo_row:
                pseudo_row["has__any_needle"] = int(any(v for k, v in pseudo_row.items() if k.startswith("has__") and k != "has__any_needle"))
            pseudo_rows.append(pseudo_row)

            synthetic_y_row_id = f"X1_{i:04d}"
            pseudo_y_rows[synthetic_y_row_id] = {
                "y_ok": True,
                "y": payload,
            }
            row_maps[i] = {
                "row_id": row_obj.get("row_id") or payload.get("row_id") or manual_y_row_id,
                "y_row_id": manual_y_row_id,
            }

        if not pseudo_rows:
            continue

        with tempfile.TemporaryDirectory(prefix=f"moltie_manual_bridge_{mode}_") as td:
            td_path = Path(td)
            pseudo_ws_path = td_path / f"ws_manual_{mode}.csv"
            pseudo_y_path = td_path / f"y_manual_{mode}.json"

            pd.DataFrame(pseudo_rows).to_csv(pseudo_ws_path, index=False)
            pseudo_y_path.write_text(
                json.dumps({"version": "manual-bridge-v1", "source": str(y_manual_path), "rows": pseudo_y_rows}, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )

            cfg = NeedleBridgeConfig(
                matches_index_csv=matches_index_csv,
                match_frequencies_csv=None,
                ws_enhanced_csv=pseudo_ws_path,
                y_inferred_json=pseudo_y_path,
                et_path_col="path",
                ws_row_id_col="X1",
        ...
        run_plan_df["row_id"] = run_plan_df["row_id"].map(lambda n: row_maps[int(n)]["row_id"])
        run_plan_df["y_row_id"] = run_plan_df["y_row_id"].map(
            lambda yid: row_maps[int(str(yid).split("_")[-1])]["y_row_id"]
        )
        all_frames.append(run_plan_df)

    return pd.concat(all_frames, ignore_index=True) if all_frames else pd.DataFrame()


def _resolve_manual_source_jobs(row_obj: dict, y_manual_path: Path) -> pd.DataFrame:
    ...
    for source_grouped_path in _candidate_manual_source_grouped_paths(row_obj, y_manual_path):
        checked_paths.append(str(source_grouped_path))
        if not source_grouped_path.exists():
            missing_paths.append(str(source_grouped_path))
            continue
    ...
    if missing_paths and len(missing_paths) == len(checked_paths):
        raise FileNotFoundError(
            "No grouped jobs source found for manual source resolution. Checked: "
            + ", ".join(missing_paths)
        )

    raise ValueError(
        "Manual Y source_inferred_rows did not match any rows in grouped jobs source. Checked: "
        + ", ".join(checked_paths)
    )
...
def create_grouped_jobs_from_manual_y(
    *,
    ...
    for y_row_id, row_obj in _iter_manual_y_records(y_manual_path):
        ...
        elif as_list(row_obj.get("source_inferred_rows")):
            bridge_candidate = True

    if bridge_candidate:
        if not selected_modes:
            raise ValueError("Select at least one precedent mode for manual needle bridging.")
        run_plan_df = _build_manual_bridge_run_plan(
            y_manual_path=y_manual_path,
            selected_modes=selected_modes,
        )
        if run_plan_df.empty:
            raise ValueError(
                "Manual Y source_inferred_rows did not produce any WS->ET matches through the needle bridge."
            )
        grouped_bridge_df = _build_grouped_jobs(run_plan_df)
        if not grouped_bridge_df.empty:
            grouped_bridge_df["y_source_path"] = str(y_manual_path)
            rows_out.extend(grouped_bridge_df.to_dict(orient="records"))

    if not rows_out:
        raise ValueError(
            "Manual Y records must include explicit PDF paths or source_inferred_rows that can be bridged "
            "via WS enhanced rows and ET needle matches."
        )
...
def apply_workflow_defaults(workflow: str) -> None:
    ...
    st.session_state["run_output_dir"] = str(cfg["out_dir"])
...
if "run_output_dir" not in st.session_state:
    st.session_state["run_output_dir"] = str(WORKFLOW_CONFIG["CSV Bridge"]["out_dir"])
...
if corpus_ready:
    st.success(
        ...
        "No rebuild is required unless your inputs changed."
    )
else:
    st.warning(
        f"No grouped corpus found for `{selected_workflow}` at `{active_grouped_path}`. "
        "Create the Y source and build the corpus below."
    )

with st.expander("Build Corpus", expanded=not corpus_ready):
    st.subheader("1. Y Creation")
    if selected_workflow == "CSV Bridge":
        default_csv_modes = _available_csv_modes() or ["offensive", "defensive"]
        st.caption("This workflow first creates row-derived Y JSON via LLM inference, then uses that Y in the corpus builder.")

        prep_modes = st.multiselect(
            "Y creation modes",
            options=["offensive", "defensive"],
            default=default_csv_modes,
            help="These control ET harvesting and CSV -> LLM -> Y generation for the CSV workflow.",
        )

        y_left, y_right = st.columns([1.1, 1.4])
        y_left.markdown("**Actions**")
        y_left.caption("Run the ET harvester first, then the CSV -> LLM inference step if those inputs are missing.")

        if y_left.button("1. Build ET Matches", key="prep_et_matches", use_container_width=True):
            if not prep_modes:
        ...
        if y_left.button("2. Build Y From CSV (LLM)", key="prep_ws_enriched", use_container_width=True):
            if not prep_modes:
                st.error("Select at least one preparation mode.")
                st.stop()
            try:
        ...
        y_right.markdown("**Dependencies**")
        y_right.caption(f"ET source root: `{ET_INPUT_ROOT}`")
        y_right.caption(f"WS source CSV: `{DEFAULT_WS_INPUT_CSV}`")
        for mode in ["offensive", "defensive"]:
            with y_right.expander(f"{mode.title()} Inputs", expanded=(mode in prep_modes and not corpus_ready)):
                y_right.caption(_path_status_line(DEFAULT_NEEDLE_JSON_BY_MODE[mode]))
                y_right.caption(_path_status_line(DEFAULT_MATCHES_INDEX_BY_MODE[mode]))
                y_right.caption(_path_status_line(DEFAULT_WS_ENHANCED_BY_MODE[mode]))
                y_right.caption(_path_status_line(DEFAULT_Y_BY_MODE[mode]))
                y_right.caption(_path_status_line(BASE_MATCHES_ROOT / mode / "_tag_spec.json"))
    else:
        y_left, y_right = st.columns([1.1, 1.4])
        y_left.markdown("**Manual JSON**")
        y_left.caption("This workflow skips CSV -> LLM inference and uses the manual Y JSON directly.")
        manual_y_path_raw = y_right.text_input(
            "Manual Y JSON",
            value=str(WORKFLOW_CONFIG["Manual JSON"]["y_path"]),
            placeholder="/home/hello/Projects/Statements/output/debug_y/Y_manual.json",
            help="The manual Y file is the Y-side source for this workflow.",
        ).strip()
        y_right.caption(_path_status_line(Path(manual_y_path_raw).expanduser()))

    st.subheader("2. Corpus Builder")
    st.caption("Build the grouped-jobs parquet that the runner will consume. This is the corpus artifact for the active workflow.")
    ...
    if selected_workflow == "CSV Bridge":
        ...
        csv_modes = create_left.multiselect(
            ...
            options=["offensive", "defensive"],
            default=default_csv_modes,
            help="Build the grouped corpus from ET matches, needle linkage, and the mode-specific inferred Y JSONs.",
        )
        create_left.text_input(
            "Y source",
            value="mode-specific automatic",
            disabled=True,
            help="CSV Bridge uses output/Y_inferred_offensive.json and output/Y_inferred_defensive.json automatically.",
        )
        ...
        with st.expander("Corpus Builder Inputs", expanded=False):
            for mode in ["offensive", "defensive"]:
                st.markdown(f"**{mode}**")
                st.code(str(DEFAULT_MATCHES_INDEX_BY_MODE[mode]))
                st.code(str(DEFAULT_WS_ENHANCED_BY_MODE[mode]))
                st.code(str(DEFAULT_Y_BY_MODE[mode]))

        if st.button("3. Build Grouped Parquet", type="secondary", key="create_corpus_csv", use_container_width=True):
            if not csv_modes:
            ...
            missing_by_mode = {mode: vals for mode, vals in missing_by_mode.items() if vals}
            if missing_by_mode:
                parts = []
                for mode, vals in missing_by_mode.items():
                    parts.append(f"{mode}: " + ", ".join(vals))
                st.error("CSV Bridge inputs are missing for the selected modes. " + " | ".join(parts))
                st.stop()
        ...
        manual_modes = create_left.multiselect(
            "Corpus modes",
            options=["offensive", "defensive"],
            default=["offensive", "defensive"],
            help="Build one manual grouped parquet that can include offensive, defensive, or both modes.",
        )
        ...
        create_left.caption(
            "The corpus builder uses the selected manual Y JSON, source_inferred_rows, WS enhanced rows, and ET needle matches to create the grouped parquet."
        )

        if st.button("2. Build Grouped Parquet", type="secondary", key="create_corpus_manual", use_container_width=True):
            try:
                grouped_df = create_grouped_jobs_from_manual_y(
                    y_manual_path=Path(manual_y_path_raw).expanduser().resolve(),
                    selected_modes=manual_modes,
...
st.divider()
st.subheader("3. Runner")
st.caption("Load the grouped parquet produced above, filter the jobs you want, then run Moltie.")
